...
class PostCarrierGroupCustomerRepository extends AbstractRepository
{
    ...
    public function getQueryBuilderBySearchData($searchData, $is_event_on = false)
    {
        ...
        // メルマガ希望フラグを無視する。または、EC本会員を除外する。
        if (!isset($searchData['ignore_permissions']) || $searchData['ignore_permissions'] === null || $searchData['ignore_permissions'] === 2) {
            ...
            // EC本会員を無視する場合は、メルマガ希望フラグに関係なく無視する。
            if ($searchData['ignore_permissions'] === null) {
                $qb3->andWhere('c3.postcarrier_flg = :c3_postcarrier_flg')
                    ->setParameter('c3_postcarrier_flg', Constant::DISABLED);
            }

            $qb
                ->andWhere($qb->expr()->not($qb->expr()->exists($qb3->getDQL()))) // NOT EXISTS メルマガ希望かEC会員でない場合、結果セットは空
                ->setParameters(new ArrayCollection(array_merge($qb->getParameters()->toArray(),
